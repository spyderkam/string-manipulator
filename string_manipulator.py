...
class Text:
    ...
    def create_smaller_files(self, NO_lines, divlines, extension='txt'):     # NO_lines = number of lines in original file; divlines ≈ how many lines in divided files
        """Divide larger file into a divlines files potentially + 1."""
        ...
        for i in range(NO_new_files):
            new_file = open(f"smaller_files/{i}_file.txt", "w")
...
if __name__ == '__main__':
    ...
    lines.create_smaller_files(NO_lines=len(file_lines), divlines=12)    # could potentially have one more than divline based on remainder
